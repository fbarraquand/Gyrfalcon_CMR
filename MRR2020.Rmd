...
OK, so we have 1816 unique IDs. Let's compare to how many gyrs have been recovered. 
...
table(drecov$Condition_Euring_code) ## 2 + 73 + 144 + 27 = 246, 8 + 46 + 21 = 75
# check category 5 (most killed)
drecov[drecov$Condition_Euring_code == 5,]
```
    ...
    for (d in 1:length(year_recov)){
        year_recov_focal = year_recov[d]-min_year+1
         ...
         if  (type_recov[d] %in% c(4,7,8)){ #live recovery
           ...
           print(c("Delta years",year_recov[d]-dringed$Ringed_year[i],"Individual",as.character(dringed$Ring_No[i]),"Status",CH[i,year_recov_focal]))
           } else {
...
### All good. 
write.csv(CH,file = "data/CaptureHistories2020.csv")


## Sort the capture histories by first capture
x=sort(dringed$Ringed_year,index.return = TRUE)
x$ix #index of rank
CHordered = CH[x$ix,]
...
pdf(file = "ObservedStates.pdf",width=10,height=8)
#layout(matrix(c(1,2,3), 3, 1, byrow = TRUE))
par(mfrow=c(3,1),mar=c(4.5,5,2,2),cex.lab = 1.5,cex.axis=1.25)#,mar=c(3,2,1,2), oma=c(0,0,0,0)
time1=1
time2=600
image.plot(time1:time2,timespan,CHordered[time1:time2,],col=cols,xlab="",ylab = "",axis.args = list(at = 1:3, labels=c('1','2','3')))
time1=601
time2=1200
image(time1:time2,timespan,CHordered[time1:time2,],col=cols,xlab="",ylab = "Time")
time1=1201
time2=nrow(dringed)
image(time1:time2,timespan,CHordered[time1:time2,],col=cols,xlab="Individual ID",ylab = "")
dev.off()
...
write.csv(stagemarking,file = "data/stagemarking2020.csv")

#We also need the year of first marking
fmarking = dringed$First_recov_year
write.csv(fmarking ,file = "data/fmarking2020.csv")
