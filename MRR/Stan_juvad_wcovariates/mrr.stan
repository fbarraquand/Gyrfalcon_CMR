...
data {
  ...
  double<lower=0> prey_abund[n_occasions-1]; 
}
...
parameters {
  real<lower=0,upper=1> mean_s[2]; // Mean survival for the two stages
...
model {
     ...
     mean_s[1] ~ uniform(0, 1); // Prior for mean juvenile survival // perhaps better explicit
     mean_s[2] ~ uniform(0, 1); // Prior for mean adult survival
